...
class DuelingQNetwork(nn.Module):
    def __init__(self, obs_dim, n_actions, hidden_dim):
        ...
        for layer in self.features:
            if isinstance(layer, nn.Linear):
                nn.init.orthogonal_(layer.weight, gain=math.sqrt(2))
                nn.init.zeros_(layer.bias)
        for head in (self.value, self.advantage):
    ...
    def forward(self, x):
        ...
        return value + advantage - advantage.mean(dim=1, keepdim=True)  # (V, A) 분해의 불확정성 제거 A의 평균을 0으로 고정 


def build_network(obs_dim, n_actions):
    network_class = DuelingQNetwork if network_type == "dueling" else QNetwork
    return network_class(obs_dim, n_actions, hidden_dim).to(device)
...
def update_parameter_with_loss():
    batch = replay_sample(batch_size)

    all_q_values = online_net(batch["states"])
    selected_q_values = all_q_values.gather(1, batch["actions"].unsqueeze(1)).squeeze(1)

    with torch.no_grad():
        next_actions = online_net(batch["next_states"]).argmax(dim=1, keepdim=True)
        next_q_values = target_net(batch["next_states"]).gather(1, next_actions).squeeze(1)
...
def run_evaluation(n_eval, eval_seed_start):
...
batch_size = 256
train_frequency = 8
target_net_update_interval = 20_000
eps_decay_episodes = 30_000
log_interval = 10_000
eval_interval_episodes = 2_50
checkpoint_interval_episodes = 5_00
progress_save_interval_steps = 500000  # 이 스텝마다 체크포인트 + 학습 곡선을 중간 저장
gif_save_interval_steps = 500000  # 이 스텝마다 고정 시드로 플레이 GIF를 중간 저장 (에피소드 롤아웃이 필요해 더 무거움)
...
# 사람 플레이 경험 (record_human_play.py로 기록) — 있으면 매 학습 배치에 항상 일정 비율로 섞임 (영구 데모 버퍼)
if demo_path.exists():
    ...
    for episode in run_bar:
        ...
        while True:
            ...
            if global_step >= next_progress_step:
                # 최종 산출물(GIF/Q-heatmap 등)까지는 아니고, 최신 체크포인트와 학습 곡선만 가볍게 중간 저장
            ...
            if global_step >= next_gif_step:
                # 고정 시드 1개를 끝까지 플레이해서 현재 정책의 GIF를 남김 (에피소드 롤아웃이 필요해 비교적 무거움)
